...
let DentistShiftViewerComponent = class DentistShiftViewerComponent {
    constructor(dentistShift, fireStore, modalService, formBuilder1, formBuilder2, reviewService, pollService, spinner) {
    ...
    tryReview() {
        ...
        if (this.form1.valid) {
            const code = this.form1.get('code').value;
            const desc = this.form1.get('descReview').value;
    ...
    tryPoll() {
        ...
        if (this.form2.valid) {
            ...
            const clinic = this.form2.get('clinic').value;
            const specialist = this.form2.get('specialist').value;
            const desc = this.form2.get('descPoll').value;
    ...
    generarPDF() {
        ...
        html2canvas__WEBPACK_IMPORTED_MODULE_10___default()(data).then(canvas => {
            ...
            let pdf = new jspdf__WEBPACK_IMPORTED_MODULE_9__('p', 'mm', 'a4'); // A4 size page of PDF  
            var position = 0;
...
DentistShiftViewerComponent.ctorParameters = () => [
...
let DentistShiftComponent = class DentistShiftComponent {
    constructor(formBuilder, dentist, dentistShift) {
...
DentistShiftComponent.ctorParameters = () => [
...
let TypeValidatorDirective = class TypeValidatorDirective {
    constructor(element, templateRef, viewContainer) {
        this.element = element;
        this.templateRef = templateRef;
        this.viewContainer = viewContainer;
    }
    set appTypeValidator(type) {
...
TypeValidatorDirective.ctorParameters = () => [
...
let DentistShiftService = class DentistShiftService {
    constructor(fireStore, authService) {
        this.fireStore = fireStore;
        this.authService = authService;
        this.dentistShifts = [];
    }
    dentistShiftRegister(dni, date, hour, specialist) {
        //Validación...
        const request = {
            dni: dni,
            date: date,
...
DentistShiftService.ctorParameters = () => [
    { type: _angular_fire_firestore__WEBPACK_IMPORTED_MODULE_3__["AngularFirestore"] },
...
let DentistService = class DentistService {
    constructor(fireStore, fireStorage, authService) {
        this.fireStore = fireStore;
        this.fireStorage = fireStorage;
        this.authService = authService;
        this.dentists = [];
...
let PollService = class PollService {
    ...
    pollRegister(dni, clinic, specialist, desc) {
        //Validación...
        const request = {
            dni: dni,
            clinic: clinic,
...
let UserService = class UserService {
    ...
    userLogin(email, password, type) {
        this.authService.signIn(email, password, type);
    }
    userRegister(dni, email, password, firstName, lastName, type, file) {
        const request = {
            dni: dni,
            email: email,
            password: password,
